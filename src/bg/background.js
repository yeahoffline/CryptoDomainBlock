...
function blockRequest (requestUrl) {
  ...
  const hostnameClean = removeDiacritics(unicodeDomain)
  const parsedClean = psl.parse(hostnameClean)

  if (parsed.sld !== parsedClean.sld && knownTLDs[parsedClean.sld] !== undefined) {
    return {
      cancel: true,
      sld: parsedClean.sld,
      tld: parsedClean.tld,
      hostname: url.hostname,
      knownTld: knownTLDs[parsedClean.sld],
      title: 'IDN homograph attack',
      message: unicodeDomain + ' looks similar to ' + parsedClean.domain + ' but transaltes to ' + parsed.domain
    }
  }
  ...
  return {
    ...
    title: parsed.sld + '.' + parsed.tld.toUpperCase() + ' blocked',
    message: 'Looking for ' + parsed.sld + '.' + knownTLDs[parsed.sld] + ' ?'
  }
}
